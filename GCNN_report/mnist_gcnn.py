            ...
            with tf.Session() as sess:
                ...
                for i in range(1, self.steps+1):
                    ...
                    if i % 100 == 0 or i == self.steps:
                        ...
                        los_test = []
                        for _ in range(10):
                            # 一次评估全部10,000个图片在CPU上资源耗尽
                            # 切分成10个每组1,000图计算后求平均，输出得到的是全部test数据集上的准确率
                            # 但是由于summary写入是通过sess.run的形式，tensorboard展示的是1,000个test数据集上的准确率
                            # 经过试验对比，两者差别不大
...
if __name__ == "__main__":
    ...
    logits = model.add_fully_connected_layer(input=fc1,
                                             ...
                                             keep_prob=1.0,
                                             scope="output_layer")
    print("%Fully_connected layer 2/Output layer: [?, 10]")
    # ---------------------------------
